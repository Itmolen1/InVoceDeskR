    ...
    public class BillsController : Controller
    {
        ...
        public ActionResult PrintReport(int ID)
        {
            try
            {
                ...
                List<QuotationReportModel> quotationReportModels = entity.BillTables.Where(x => x.BilID == ID).Select(x => new QuotationReportModel
                {

                    QutationID = x.BilID,
                    Qutation_ID = x.Bill_ID,
                    RefNumber = x.RefNumber,
                    QutationDate = x.BillDate.ToString(),
                    DueDate = x.BillDueDate.ToString(),
                ...
                BillTable qt = entity.BillTables.Where(x => x.BilID == ID).FirstOrDefault();
                ...
                List<GoodsTable> goodsTable = entity.BillDetailTables.Where(x => x.BillID == ID && x.Type == "Goods").Select(x => new GoodsTable
                {
                ...
                List<ServicesTables> servicesTabless = entity.BillDetailTables.Where(x => x.BillID == ID && x.Type == "Service").Select(x => new ServicesTables
                {
                ...
                Report.Load(Server.MapPath("~/CrystalReport/Bills.rpt"));
